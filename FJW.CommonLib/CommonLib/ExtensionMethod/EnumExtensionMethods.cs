...
namespace FJW.CommonLib.ExtensionMethod
{
    ...
    public static class EnumExtensionMethods
    {
        ...
        {
            var t = pCaller.GetType();//获取枚举上的描述特性 
            var fi = t.GetField(Enum.GetName(t, pCaller));
            var att = (DescriptionAttribute)Attribute.GetCustomAttribute(fi, typeof(DescriptionAttribute));
            
            if (att != null && !string.IsNullOrEmpty(att.Description))//如果有描述特性且值不为空则返回描述否则返回枚举的值 
